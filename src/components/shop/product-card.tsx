...
export function ProductCard({ product }: { product: ShopProduct }) {
  ...
  return <article className="shop-product-card">
    <Link href={`/products/${product.slug}`} className="shop-product-visual" aria-label={`View ${product.name}`}>
      ...
      <Image src={product.image} alt={`${product.name} ${product.size} bottle`} width={380} height={420} sizes="(max-width: 600px) 85vw, (max-width: 1000px) 45vw, 30vw" unoptimized={product.image.startsWith('/api/media') || product.image.startsWith('http')} />
